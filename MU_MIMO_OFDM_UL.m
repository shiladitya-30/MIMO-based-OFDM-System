...
for it=1:lrg_itrns
    ...
    %Small Scale Fading>>>>>>>>>>>>>>
    for jj=1:sml_itrns
        hpre=(randn(Nr,nUEs,Ch_taps)+1j.*randn(Nr,nUEs,Ch_taps)).*(reshape(sqrt(var/2),1,1,[]));
        chnl{it,jj,combiner}=hpre;
...
plot(SNR_db,SE(1,:),'LineWidth', 1.8, 'Marker', 'square', 'MarkerSize', 8 , 'MarkerFaceColor','g');
hold on;
plot(SNR_db,SE(2,:),'LineWidth', 1.8, 'Marker', 'square', 'MarkerSize', 8 , 'MarkerFaceColor','b');
plot(SNR_db,SE(3,:),'LineWidth', 1.8, 'Marker', 'square', 'MarkerSize', 8 , 'MarkerFaceColor','c');
