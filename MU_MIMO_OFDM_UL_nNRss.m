...
nUEs = 4; %Total Users
nNrs = 16:16:300; %Total BS Antennas
No=1;
...
lrg_itrns=7;sml_itrns=7;uu_itrns=7;

for it=1:lrg_itrns
    for jj=1:sml_itrns
        htotl{it,jj}=(randn(nNrs(end),nUEs,Ch_taps)+1j.*randn(nNrs(end),nUEs,Ch_taps)).*(reshape(sqrt(var/2),1,1,[]));
    end
end
...
for it=1:lrg_itrns
    ...
    beta_save{it,:,combiner}=beta_values;
    Beta=diag(beta_values);

    %Small Scale Fading>>>>>>>>>>>>>>
    for jj=1:sml_itrns
        for rr=1:length(nNrs)
            [combiner,uu,it,jj,rr]
            htemp=htotl{it,jj};
            hpre=htemp(1:nNrs(rr),:,:);
            ...
            p_user=SNR/nUEs;%Equal Power
            % Combiner>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            W1=zeros(nUEs,nNrs(rr),nSubC);
            %MRC>>>>>>>
            if combiner==1  
                for m=1:nSubC
                    W1(:,:,m)=Hm(:,:,m)';   
            ...
            elseif combiner==3 
               for m=1:nSubC
                    W1(:,:,m)=inv(((Hm(:,:,m)')*Hm(:,:,m))+((No/SNR)*eye(nUEs)))*(Hm(:,:,m)');
               end
            end
...
plot(nNrs,SE(1,:),'LineWidth', 1.6, 'Marker', 'square', 'MarkerSize', 6 , 'MarkerFaceColor','g');
hold on;
plot(nNrs,SE(2,:),'LineWidth', 1.6, 'Marker', 'square', 'MarkerSize', 6 , 'MarkerFaceColor','b');
plot(nNrs,SE(3,:),'LineWidth', 1.6, 'Marker', 'square', 'MarkerSize', 6 , 'MarkerFaceColor','c');
